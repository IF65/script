...
ALTER TABLE TEMP.NEWOFFERTEINCORSO ADD PRIMARY KEY(ARTICOLO,DATAIN,DATAFI,IDPROMO), ENGINE=MYISAM;
...
-- select      distinct
--             @mystm:=  concat("SELECT RIEPVEGI_RECORD from TEMP.OUTRIEPVEGI into outfile 'C:/Italmark/ETL/NCR/Datacollect/Riepvegi/Temp/NUL_NO-EMAIL_",
...
delimiter $$
drop procedure if exists funzioni.split_RIEPVEGI$$
create procedure funzioni.split_RIEPVEGI()         
BEGIN
  DECLARE done INT DEFAULT FALSE;
  DECLARE RUNA_TIMESTAMP CHAR(14);
  DECLARE a CHAR(4);
  DECLARE NOMEFILE VARCHAR(100);  
  DECLARE cur1 CURSOR FOR SELECT distinct NEGOZIO FROM TEMP.OUTRIEPVEGI;
  DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
  SET RUNA_TIMESTAMP=LEFT((NOW() + 0),14);
  OPEN cur1;
  read_loop: LOOP
    FETCH cur1 INTO a;
    IF done THEN
      LEAVE read_loop;
    END IF;
    SET NOMEFILE = CONCAT("NUL_NO-EMAIL_",RUNA_TIMESTAMP ,"_DC-ASAR-RIEPVEGI_", a ,"_", (SELECT MAX(DATA) FROM TEMP.OUTRIEPVEGI WHERE NEGOZIO = a),".TXT");
    insert INTO  trasmissioni.job (IDHOST,TIPO,USER,PASSWORD,LOC_DIR,LOC_FILE,ACTION,REM_DIR,REM_FILE,ENABLED,TODO)
    select idhost,
           10,
           'italfrutta',
           'italfrutta',
           'e:/IT/BACKUP/RIEPVEGI/',
           NOMEFILE,
           'UPLOAD',
           'hocidc/Riepvegi/',
           CONCAT("D",substr(RUNA_TIMESTAMP,4,7),".",substr(RUNA_TIMESTAMP,11,2),"C"),
           CASE WHEN a > "0100" and a < "0200" then 1 else 0 end,
           CASE WHEN a > "0100" and a < "0200" then 1 else 0 end
    FROM   TRASMISSIONI.HOST
    WHERE  SUBSTR(a,1,2) = societa
    and    SUBSTR(a,3,2) = negozio
    and    societa = "01"
    and    abilita = 1; 
    select      distinct
                @myvar:=  concat("SELECT RIEPVEGI_RECORD from TEMP.OUTRIEPVEGI into outfile 'C:/IT/ETL/NCR/Datacollect/Riepvegi/Temp/",
                                NOMEFILE,"'")
    FROM        TEMP.OUTRIEPVEGI;
