...
# bisogna usare gli escape x i caratteri speciali
UTENTE="supermedia"
PASSWORD="2x_P\%q97Nr\*t"
FILE_BRANDON="$DIR/BRANDON_Catalogo.txt"
FILE_FTP=Catalogo_Supermedia.txt
if [ -s "$FILE_TEKWORLD" ]; then
   	ftp -in 52.213.47.212 1>/dev/null <<SCRIPT
		user $UTENTE $PASSWORD
		cd IN
		binary
		put $FILE_BRANDON $FILE_FTP
		bye
SCRIPT
fi
...
#BODY="<html><body>\n<b>AGGIORNAMENTO CATALOGO B2B<BR>$DATA</b><BR><BR>\n"
#SUBJECT="AGGIORNAMENTO CATALOGO B2B"

#/sendEmail-v1.56/sendEmail -o tls=no -u $SUBJECT -m $BODY -f edp@if65.it -t $ADDRESS -s 10.11.14.234:25 -a $FILE_EPRICE $FILE_ONLINESTORE $FILE_YEPPON $FILE_Supermedia $FILE_TEKWORLD
