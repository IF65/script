...
$logger->info("-" x 76);
$logger->info("calcolo forme di pagamento");
$logger->info("-" x 76);
$logger->info("inizio");
...
if(&ConnessioneDB()) {
	...
	if (my $db = new ITM_lavori(default_starting_date => $starting_date)) {
		...
		my ($_log_codice_sede, $_log_data, $_codice, $_ip, $_utente, $_password, $_percorso) = $db->elenco_forme_pagamento_da_calcolare();
			...
			my $nome_file = $$_log_codice_sede[$i].'_'.string2date($$_log_data[$i])->ymd('').'_'.substr(string2date($$_log_data[$i])->ymd(''),2).'_DC.TXT';
				...
				if (-e $datacollect_folder.'/'.$nome_cartella.'/'.$nome_file) {
					if(&analisi_file($datacollect_folder.'/'.$nome_cartella,$nome_file)) {
						...
						$db->calcolo_forme_pagamento_ok($$_log_data[$i], $$_log_codice_sede[$i]);
					}
...
sub ConnessioneDB {
    ...
    $sth = $dbh->prepare(qq{ CREATE TABLE IF NOT EXISTS `$database`.`$table` (
							...
							`pos_id` varchar(2) NOT NULL DEFAULT '',
							`carta` varchar(13) NOT NULL DEFAULT '',
							`ora` varchar(6) NOT NULL DEFAULT '',
							`importo` float NOT NULL DEFAULT '0',
							`totale` float NOT NULL,
							PRIMARY KEY (`data`,`negozio`,`cassa`,`transazione`,`pos_id`),
							KEY `carta` (`carta`)
    ...
    $sth = $dbh->prepare(qq{insert into `$database`.`$table` (`data`, `negozio`, `cassa`, `transazione`, `pos_id`, `carta`, `ora`, `importo`, `totale`)
                        values (?,?,?,?,?,?,?,?,?) on duplicate key update `ora`=?, `importo`=?, `totale`=?}) ;
		...
		while (!eof($file_handler)) {
			...
			if ($line =~ /^\d{4}:(\d{3}):\d{6}:(\d{6}):(\d{4}):.{3}:F:1.{33}(.{10})$/) {
				...
				for(my $i=0;$i<@transazione;$i++) {
					if ($transazione[$i] =~ /:T:1.{25}(\d\d).{6}(.{10})$/) {
						if (! exists $forma_pagamento{$1}) {
							$forma_pagamento{$1}=$2*1
						} else {
							$forma_pagamento{$1}+=$2*1
						}
					}
                ...
                my @keys = keys %forma_pagamento;
                for (my $i=0;$i<@keys;$i++) {
                    $sth->execute($data, $negozio, $cassa, $numero_transazione, $keys[$i],$carta, $ora, $forma_pagamento{$keys[$i]}, $totale_transazione,$ora, $forma_pagamento{$keys[$i]}, $totale_transazione);
                }
			} 
		}
		close($file_handler);
...
sub log_file_name{
    return "$log_folder/".$current_date->ymd('').'_'.$current_time->hms('')."_forme_di_pagamento.log";
}
