...
my $ftp_url = '10.11.14.154';
my $ftp_user = 'if65';
my $ftp_password = 'if65';
...
if ($help) {
    print "Utilizzo: perl [path]rvg2brix.pl -d yyyy-mm-dd [yyyy-mm-dd] [-s ssnn [ssnn [ssnn....[....]]]] [--ftp] [--flag] [--delete] [--help]\n\n";
    ...
    print "                              sono solo 01, 04, 31 e 36 se nessuna sede e\' indicata e/o l'opzione e\' mancante si considerano tutte le\n";
                    ...
                    if (open my $file_handler, "+>:crlf", "$export_folder/$nome_file\.txt") {
                        ...
                        if ($ftp) {
                            ...
                            if ( $ftph->login("$ftp_user","$ftp_password")) {
                                ...
                                if ($ftph->put("$export_folder/$nome_file\.txt")) {
                                    if ($flag) {
...
sub ConnessioneDB {
    ...
    if ($flag) {
        ...
        $sth_lavori = $dbh_lavori->prepare(qq{update incarichi as i set i.eseguito = 1 where i.lavoro_codice = 140 and i.negozio_codice = ? and i.data = ?});
    }
...
sub log_file_name {
    return "$log_folder/".$current_date->ymd('')."_invio_rvg_2_brix.log";
}
