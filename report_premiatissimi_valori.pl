...
for(my $i=0;$i<@ar_codici_padre;$i++) {
	...
	my @ar_data		= ();
	my @ar_quantita = ();
	$sth = $dbh->prepare(qq{select `RVG-CODSOC`, `RVG-CODNEG`,date_format(`RVG-DATA`,'%Y%m'), sum(`RVG-QTA-USC`) from `$database_archivi`.`$table_riepilogo` where `RVG-CODICE` in $elenco_codici group by 1,2,3 order by 1,2,3});
	if ($sth->execute()) {
...
$sth = $dbh->prepare(qq{select concat(n.societa,' - ',n.societa_descrizione), concat( n.negozio,' - ',n.negozio_descrizione), p.anno_mese, p.codice, r.descrizione, sum(p.venduto_quantita), round(sum(p.venduto_quantita*r.contributo),2),sum(p.venduto_quantita*r.punti) from `$database_catalogo`.`$table_report` as p, `$database_archivi`.`$table_negozi` as n, `$database_catalogo`.`$table_premi` as r where r.codice = p.codice and p.negozio = n.codice group by 1,2,3,4 order by 1,2,3;});
if ($sth->execute()) {
	print $output_file_handler "Societˆ'\tNegozio\tData\tCod.Art.\tDescrizione\tContr.S/N\tN.Premi\tImporto\tPunti\n";
	while(my @row = $sth->fetchrow_array()) {
