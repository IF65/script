...
sub DBConnection{
	...
	$sth_get_ep_promo_2_ref = $dbp->prepare(qq{select p.`codice_campagna`, p.`codice_promozione`, p.`classe`
						...
						p.`parametro_02` = ? and p.`codice_promozione`=n.`promozione_codice` and n.`negozio_codice`= ?
						order by p.`classe`});
...
sub C_Record() {
	...
	if ($dbrecord{'code3'} eq 5) {
		...
		if ($promo_list =~ /C0027/ or $promo_list =~ /C0024/) {
			...
			my $percentuale = 50;
			my $tipoBollone = 0;
			if ($ean_ultima_vendita eq '9980110002007') {# bollone catalina 30% (aprile 2018)
				$percentuale = 30;
				$tipoBollone = 1;
			} else {
			...
			$movimento{'movimento'}			= 13;
			($movimento{'codice_campagna'},$movimento{'codice_promozione'}) =
						&GetCodicePromozione('13',$dbrecord{'data'},'',$dbrecord{'socneg'},$percentuale, $tipoBollone);
			...
			if ($movimento{'codice_campagna'} =~ /     /) {
				...
				if ($valore_ultima_vendita != 0) {
					...
					($movimento{'codice_campagna'},$movimento{'codice_promozione'}) =
						&GetCodicePromozione('13',$dbrecord{'data'},$movimento{'codice_ean'},$dbrecord{'socneg'}, $percentuale, $tipoBollone);					
				}
			}
		}
...
sub GetCodicePromozione() {
	my ($tipo_movimento, $data_movimento, $barcode, $negozio, $percentuale, $tipoBollone) = @_;
	...
	if ($tipo_promozione eq 'BJ' or $tipo_promozione eq 'AP' or $tipo_promozione eq 'BM' or $tipo_promozione eq 'BP' or $tipo_promozione eq 'DM') {
		...
		# cerco la promozione
		if ($sth_get_ep_promo_2_ref->execute($data_promozione, $data_promozione, 'MP', $parametro_01, $tipoBollone, $negozio)) {
