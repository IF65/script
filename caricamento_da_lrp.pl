...
my $starting_date = $current_date->add(days =>-21)->ymd('/');
...
sub ConnessioneDB{
	...
	$sth = $dbh->prepare(
							qq{
								create table if not exists $database.testate_lrp (
								...
								primary key  (`id`), KEY `data` (`data`,`negozio`,`cassa`,`transazione`,`carta`)) engine=InnoDB default charset=latin1;
							});
