...
if ($db = new ITM_lavori(default_starting_date => $starting_date)) {
	...
	my ($_log_codice, $_log_data, $_codice, $_ip, $_utente, $_password, $_percorso) = $db->elenco_journal_da_caricare();
...
sub imposta_flag_presenza_file() {
	...
	for (my $i=0; $i<@date; $i++) {
		if (opendir my $DIR, "$journal_folder/".string2date($date[$i])->ymd('')) {
			my @elenco_file = sort grep { /^.*\.CTL$/ } readdir $DIR;
			closedir $DIR;
			foreach (@elenco_file) {
				my $file_ctl = $_;
				my $file_jrn = $file_ctl;
				$file_jrn =~ s/\.CTL$/\.JRN/ig; 
				unlink "$journal_folder/".string2date($date[$i])->ymd('')."/$file_ctl";
				if (-e "$journal_folder/".string2date($date[$i])->ymd('')."/$file_jrn") {
					unlink "$journal_folder/".string2date($date[$i])->ymd('')."/$file_jrn";
					$logger->warn("File di controllo $file_ctl ancora presente. File $file_jrn eliminato!");
				}
			}
		}
	}
	...
	#i file sono nella forma: ssnn_yyyymmdd_yymmdd_DC.JRN (es. 0171_20151211_151211_DC.JRN)
	for (my $i=0; $i<@$_log_codice; $i++) {
		my $giornata = "$journal_folder/".string2date($$_log_data[$i])->ymd('');
		if (! -e $giornata) {
			if (! mkdir $giornata) {
				$logger->error("Impossibile creare la cartella: $giornata, $!");
			}
		}
		my $file = $giornata."/".$$_log_codice[$i].'_'.string2date($$_log_data[$i])->ymd('').'_'.substr(string2date($$_log_data[$i])->ymd(''),2).'_DC.JRN';
		if (-e $giornata && -e $file ) {
			$db->journal_ok($$_log_data[$i], $$_log_codice[$i]);
		};
	}
