...
if ($db = new ITM_lavori(default_starting_date => $starting_date)) {
	...
	my ($_log_codice, $_log_data, $_codice, $_ip, $_utente, $_password, $_percorso) = $db->elenco_anagdafi_da_caricare();
		...
		if ($ftp->login($utente,$password)) { 
			...
			for (my $i=0;$i<@log_codice;$i++) {
				if ($negozio eq $log_codice[$i]) {
					...
					my $nome_file_semaforo = substr($data->ymd('').'_A.CTL',2);
...
sub imposta_flag_presenza_file() {
	...
	for (my $i=0; $i<@date; $i++) {
		if (opendir my $DIR, "$anagdafi_folder/".string2date($date[$i])->ymd('')) {
			...
			foreach (@elenco_file) {
				...
				unlink "$anagdafi_folder/".string2date($date[$i])->ymd('')."/$file_ctl";
				if (-e "$anagdafi_folder/".string2date($date[$i])->ymd('')."/$file_zip") {
					unlink "$anagdafi_folder/".string2date($date[$i])->ymd('')."/$file_zip";
	...
	for (my $i=0; $i<@$_log_codice; $i++) {
		my $giornata = "$anagdafi_folder/".string2date($$_log_data[$i])->ymd('');
		if (! -e $giornata) {
			if (! mkdir $giornata) {
		...
		if (-e $giornata && -e $file ) {
			$db->anagdafi_ok($$_log_data[$i], $$_log_codice[$i]);
		};
	}
