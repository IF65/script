...
$logger->info("-" x 76);
$logger->info("invio rvg ai negozi");
$logger->info("-" x 76);
...
if ($help) {
    print "Utilizzo: perl [path]rvg2shop.pl -d yyyy-mm-dd [yyyy-mm-dd] [-s ssnn [ssnn [ssnn....[....]]]] [--ftp] [--flag] [--delete] [--help]\n\n";
							...
							if ($sth_shop->execute($elenco_negozi[$i])) {
								...
								if ($ftph) {
									...
									if ( $ftph->login("$ftp_user","$ftp_password")) {
										...
										if ($elenco_negozi[$i] =~ /^3/  && $elenco_negozi[$i] !~ /^3654/ && $elenco_negozi[$i] !~ /^3694/ && $elenco_negozi[$i] !~ /^3671/) {
											$ftph->cwd("richneg/hocidc/riepvegi")
										} else {
											$ftph->cwd("hocidc/riepvegi")
										}
										$ftph->binary();
										
										my $current_date_time 	= DateTime->now(time_zone=>'local')->iso8601();
										$current_date_time =~ s/\-//ig;
										$current_date_time =~ s/\://ig;
										my $file_name = 'D'.substr($current_date_time,3,5).substr($current_date_time,9,2).'.'.substr($current_date_time,11,2).'C';
										
										$logger->debug("tentativo di invio del file: $export_folder/$nome_file\.txt");
										if ($ftph->put("$export_folder/$nome_file\.txt", $file_name)) {
											$logger->debug("inviato il file: $export_folder/$nome_file\.txt");
											if ($flag) {
...
sub ConnessioneDB {
    ...
    $sth = $dbh->prepare(qq{select
								...
								case when a.`REPA-ART2` is null then '01' else lpad(a.`REPA-ART2`,2,'0') end,
                            ...
                            from archivi.riepvegi as r left join articox2 as a on r.`RVG-CODICE`=a.`COD-ART2`
    ...
    if ($flag) {
        ...
        $sth_lavori = $dbh_lavori->prepare(qq{update incarichi as i set i.eseguito = 1 where i.lavoro_codice = 130 and i.negozio_codice = ? and i.data = ?});
    }
