...
if (&ConnessioneDB) {
    ...
    for (my $i = 0; $i < @elencoClienti; $i++) {
        ...
        $report->write( $riga, 7, 'Prezzo eprice', $formatTitoli);
        ...
        $report->write( $riga, 14, 'Prezzo Promozione ePRICE', $formatTitoli);
        ...
        if ($sth->execute($elencoClienti[$i], $elencoClienti[$i])) {
            while (my @record = $sth->fetchrow_array()) {
                ...
                $report->write_string( $riga, 4, $ediel03, $formatEmptyLeft);
...
sub ConnessioneDB {
    ...
    $sth = $dbh->prepare(qq{select 
                            ...
                            where c.`codiceCliente`= ? and (((t.`inOrdine`<>0 or t.`giacenza`<>0) and t.`canale` = 1 and t.`ordinabile` = 1) or (t.`codice` in (select distinct p.`codiceArticolo` from politicaVendita as p where p.`codiceCliente`=? and p.`codiceArticolo`<>''))) and t.`doppioNetto`<>0 and
                            t.`marchio` not in ('ASK','BSE','CLL','EXT','GOP','LIE','LOE','MIE','NIK','NRD','NTM','SBS','SNS','TRB','YAM','EAS','MEP','CAO','BLC','VIT','FAB','ACT','ADI','APO','AQL','ASF','ATT','BKB','BLC','BLR','BLJ','BNM','BOC','BPO','CEL','WEB','WAR','WAM','FOX') and
                            t.`marchioCopre` not in ('MIELE') and t.`doppioNetto`>= 5.00;
                });
