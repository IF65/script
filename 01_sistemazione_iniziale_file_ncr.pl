		...
		if (open my $old_file_handler, "<:crlf", "$path/$file_name") {
			...
			while (!eof($old_file_handler)) {
				...
				if ($line =~ /^.{31}:F:1.{43}$/) {
					...
					#sistemo bollone catalina sconto 30% (aprile 2018)
					for (my $i = 0;$i < @transazione; $i++ ) {
						if ($transazione[$i] =~ /^.{31}:S:1.{2}:(?:\s|\d){4}:\s{3}(9770110003000).*$/) {
								if ($transazione[$i] =~ /^(.{46}).{13}(.*$)/) {
									$transazione[$i] = $1.'9980110002007'.$2;
								}
						}
					};
				...
				for (my $i = 0;$i < @transazione; $i++ ) {
					...
					#individuo il reso 0492 per articoli a pezzo
					for (my $i = 0; $i < @transazione;$i++) {
						if ($transazione[$i] =~ /(:S:1)(?:7|8)(.{23})\-(.{18})$/) {
							my $prima_riga = $1.'0'.$2.'+'.$3;
							if ($transazione[$i+2] =~ /(:C:14)2(.{22})\-(.{8})\+(.{9})$/) {
								my $seconda_riga = $1.'3'.$2.'+'.$3.'-'.$4;

								if ($transazione[$i+3] =~ /:S:17.{10}998011/) {
									#eseguo lo storno
									my $stornoEseguito = 0;
									for (my $j = 0; $j < $i  && ! $stornoEseguito ;$j++) {
										if ($transazione[$j] ne '' && substr($transazione[$j], 31) eq $prima_riga && substr($transazione[$j+4], 31) eq $seconda_riga && $transazione[$j+2] =~ /:S:1.{11}998011/ ) {
											$transazione[$i] = '';
											$transazione[$i + 1] = '';
											$transazione[$i + 2] = '';
											$transazione[$i + 3] = '';
					...
					#individuo il reso 0492 per articoli a peso
					for (my $i = 0; $i < @transazione;$i++) {
						if ($transazione[$i] =~ /(:S:1)(?:7|8)(.{23})\-(.{18})$/) {
							my $prima_riga = $1.'0'.$2.'+'.$3;
							$prima_riga =~ s/\-/\+/ig;
					...
					#individuo il reso 0493 (il record m0493 viene aggiunto se non c'Ë)
					for (my $i = 0; $i < @transazione;$i++) {
						if ($transazione[$i] =~ /(:S:1)(?:7|8)(.{23})\-(.{18})$/) {
							my $prima_riga = $1.'0'.$2.'+'.$3;
							if ($transazione[$i+2] =~ /(:C:14(?:2|3).{22})\-(.{8})\+(.{9})$/) {
								my $seconda_riga = $1.'+'.$2.'-'.$3;

								if ($transazione[$i+3] =~ /:m:1.{7}:0493/) {
									#eseguo lo storno
									my $stornoEseguito = 0;
									for (my $j = 0; $j < $i  && ! $stornoEseguito ;$j++) {
										if ($transazione[$j] ne '' && substr($transazione[$j], 31) eq $prima_riga && substr($transazione[$j+2], 31) eq $seconda_riga && $transazione[$j+3] =~ /:m:1.{7}:0493/ ) {
											$transazione[$i] = '';
											$transazione[$i + 1] = '';
											$transazione[$i + 2] = '';
											$transazione[$i + 3] = '';
...
# 								}
					...
					# non serve perché ci potrebbero essere transazioni con reparti tipo 0016
					# my $repartiBrixAttivi = 0;
# 					for(my $i = 0; $i < @transazione; $i++) {
# 						if ($transazione =~ /:S:1..:.[^0]/) {
# 							$repartiBrixAttivi = 1;
# 						}
# 					}

					#conversione_reparto
					if ($negozio =~ /^(3|04|0133|0139)/) {
						for(my $i = 0; $i < @transazione; $i++) {
							if ($transazione[$i] =~ /^(.{31}:S:...:)(\d{4})(.*)$/) {
								my $testata_linea	= $1;
								my $reparto			= $2;
								my $coda_linea		= $3;
								if ($reparto eq '0016') {$reparto = '0901'};
					...
					#sistemo i reparti non previsti
					# for(my $i = 0; $i < @transazione; $i++) {
# 						if ($transazione[$i] =~ /^(.{31}:S:...:)(\d{4})(.*)$/) {
# 							my $testata_linea	= $1;
# 							my $reparto			= $2*1;
...
#
# 							if ($reparto > 9) {
# 								$transazione[$i] = $testata_linea.'0001'.$coda_linea;
# 							}
# 						}
# 					}
...
sub init_conversione_reparto() {
	...
	push(@ar_sottoreparti_brix, '0901');
	push(@ar_reparti_brix, '09');
	push(@ar_codici_brix, '9950001');
